Imports System.Web.Script.Serialization
Partial Class EF_mktMonths
  Inherits SIS.SYS.UpdateBase
  Public Property Editable() As Boolean
    Get
      If ViewState("Editable") IsNot Nothing Then
        Return CType(ViewState("Editable"), Boolean)
      End If
      Return True
    End Get
    Set(ByVal value As Boolean)
      ViewState.Add("Editable", value)
    End Set
  End Property
  Public Property Deleteable() As Boolean
    Get
      If ViewState("Deleteable") IsNot Nothing Then
        Return CType(ViewState("Deleteable"), Boolean)
      End If
      Return True
    End Get
    Set(ByVal value As Boolean)
      ViewState.Add("Deleteable", value)
    End Set
  End Property
  Public Property PrimaryKey() As String
    Get
      If ViewState("PrimaryKey") IsNot Nothing Then
        Return CType(ViewState("PrimaryKey"), String)
      End If
      Return True
    End Get
    Set(ByVal value As String)
      ViewState.Add("PrimaryKey", value)
    End Set
  End Property
  Protected Sub ODSmktMonths_Selected(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.ObjectDataSourceStatusEventArgs) Handles ODSmktMonths.Selected
    Dim tmp As SIS.MKT.mktMonths = CType(e.ReturnValue, SIS.MKT.mktMonths)
    Editable = tmp.Editable
    Deleteable = tmp.Deleteable
    PrimaryKey = tmp.PrimaryKey
  End Sub
  Protected Sub FVmktMonths_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles FVmktMonths.Init
    DataClassName = "EmktMonths"
    SetFormView = FVmktMonths
  End Sub
  Protected Sub TBLmktMonths_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles TBLmktMonths.Init
    SetToolBar = TBLmktMonths
  End Sub
  Protected Sub FVmktMonths_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles FVmktMonths.PreRender
    TBLmktMonths.EnableSave = Editable
    TBLmktMonths.EnableDelete = Deleteable
    Dim mStr As String = ""
    Dim oTR As IO.StreamReader = New IO.StreamReader(HttpContext.Current.Server.MapPath("~/MKT_Main/App_Edit") & "/EF_mktMonths.js")
    mStr = oTR.ReadToEnd
    oTR.Close()
    oTR.Dispose()
    If Not Page.ClientScript.IsClientScriptBlockRegistered("scriptmktMonths") Then
      Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "scriptmktMonths", mStr)
    End If
  End Sub

End Class
